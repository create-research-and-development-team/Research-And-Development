package org.valkyrienskies.vsrnd.util.compat;
...
public class CompatUtil {
	...
	public static Vector3d toSameSpaceAs(Level level, Vector3d position, Vector3d target) {
		Ship ship;
		if (VSGameUtilsKt.isBlockInShipyard(level, position.x, position.y, position.z) &&
			!VSGameUtilsKt.isBlockInShipyard(level, target.x, target.y, target.z)) {
