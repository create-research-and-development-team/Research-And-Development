...
public class FermentingTankItem_FORGE extends FermentingTankItem {
	...
	private void tryMultiPlace(BlockPlaceContext ctx) {
		...
		FermentingTankBlockEntity_FORGE tankAt = ConnectivityHandler.partAt(VSCreateForgeBlockEntities.FERMENTING_TANK.get(),
																			world,
																			placedOnPos
																		   );
		if (tankAt == null) {
		...
		FermentingTankBlockEntity_FORGE controllerBE = tankAt.getControllerBE();
		...
		for (int xOffset = 0; xOffset < width; xOffset++) {
			for (int zOffset = 0; zOffset < width; zOffset++) {
				...
				if (FermentingTankBlock_FORGE.isTank(blockState)) {
					continue;
				}
	...
	protected boolean updateCustomBlockEntityTag(BlockPos p_195943_1_, Level p_195943_2_, Player p_195943_3_,
		...
		if (nbt != null) {
			...
			if (nbt.contains("TankContent")) {
				...
				if (!fluid.isEmpty()) {
					fluid.setAmount(Math.min(FermentingTankBlockEntity_FORGE.getCapacityMultiplier(),
											 fluid.getAmount()));
